				...
				<uni-section title="老年肌少症问卷"
					...
					<uni-forms label-position="top" :modelValue="sarcopeniaForm" ref="form1" labelWidth="auto">
						<uni-forms-item label="1、您提起或者搬运10斤物品有多大困难" required>
							<uni-data-checkbox v-model="sarcopeniaForm.handling" :localdata="selectedList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="2、您步行穿过一个房间有多大困难" required>
							<uni-data-checkbox v-model="sarcopeniaForm.walking" :localdata="selectedList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="3、您从椅子或床边站起有多大困难" required>
							<uni-data-checkbox v-model="sarcopeniaForm.standUp" :localdata="selectedList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="4、您上10个台阶有多大困难" required>
							<uni-data-checkbox v-model="sarcopeniaForm.onSteps" :localdata="selectedList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="5、您在过去的一年里跌倒了多少次" required>
							<uni-data-checkbox v-model="sarcopeniaForm.fallNumber" :localdata="fallNumber" :disabled="flag =='look'"/>
						</uni-forms-item>
					</uni-forms>
				</uni-section>
			</view>
				...
				<uni-section title="危险因素测试" subTitle="国际骨质疏松症基金会（IOF）一分钟危险因素测试,一道题答“是”即为高危人群" type="line" padding>
					...
					<uni-forms label-position="top" :modelValue="riskFactorsForm" ref="form4" labelWidth="auto">
						<uni-forms-item label="1、您的父母有骨质疏松病史或轻微跌倒后发生骨折的经历" required>
							<uni-data-checkbox v-model="riskFactorsForm.form1" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="2、您的父母是否有驼背" required>
							<uni-data-checkbox v-model="riskFactorsForm.form2" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="3、实际年龄超过40岁" required>
							<uni-data-checkbox v-model="riskFactorsForm.form3" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="4、您在成年后是否曾经因为轻微的碰撞就发生骨折" required>
							<uni-data-checkbox v-model="riskFactorsForm.form4" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="5、您过去1年有跌倒的经历吗？或者您因为身体虚弱而害怕跌倒" required>
							<uni-data-checkbox v-model="riskFactorsForm.form5" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="6、在您40岁后，身高是否比年轻时降低了超过3厘米" required>
							<uni-data-checkbox v-model="riskFactorsForm.form6" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="7、您的体重是否较低（体重指数<19kg/m2）" required>
							<uni-data-checkbox v-model="riskFactorsForm.form7" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="8、您是否曾连续3个月以上服用“可的松、强的松”等激素类药品" required>
							<uni-data-checkbox v-model="riskFactorsForm.form8" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="9、您是否有类风湿性关节炎病史" required>
							<uni-data-checkbox v-model="riskFactorsForm.form9" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="10、您是否有甲状腺功能亢进、甲状旁腺功能亢进、1型糖尿病、营养或胃肠道功能障碍（克罗恩病、腹部疾病）病史" required>
							<uni-data-checkbox v-model="riskFactorsForm.form10" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="11、（女士回答） 您是否在45岁以前就绝经了" required>
							<uni-data-checkbox v-model="riskFactorsForm.form11" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="12、（女士回答） 您是否曾经有过连续12个月以上没有月经？（绝经、怀孕、子宫切除除外）" required>
							<uni-data-checkbox v-model="riskFactorsForm.form12" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="13、（女士回答）50岁前，您是否做过卵巢切除，且没有接受激素替代治疗" required>
							<uni-data-checkbox v-model="riskFactorsForm.form13" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="14、（男士回答）您是否患有阳痿或缺乏性欲等雄激素缺乏的症状" required>
							<uni-data-checkbox v-model="riskFactorsForm.form14" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="15、您经常大量饮酒吗" required>
							<uni-data-checkbox v-model="riskFactorsForm.form15" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="16、您目前或曾经一段时间吸烟吗" required>
							<uni-data-checkbox v-model="riskFactorsForm.form16" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="17、您每天的体力活是否少于30分钟（家务、养花、走路、跑步）" required>
							<uni-data-checkbox v-model="riskFactorsForm.form17" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="18、您是否不喝牛奶或奶制品，或对奶制品过敏，且未补充钙剂" required>
							<uni-data-checkbox v-model="riskFactorsForm.form18" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="19、您每天的户外活动是否少于10分钟，且未补充维生素D" required>
							<uni-data-checkbox v-model="riskFactorsForm.form19" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
					</uni-forms>
				</uni-section>
			</view>
					...
					<uni-forms :modelValue="OSTAForm" ref="form4" labelWidth="120px">
						<uni-forms-item label="体质量（kg）" required>
							<uni-easyinput v-model="OSTAForm.weight" :inputBorder="false" placeholder="请输入体质量"
								@iconClick="clickWeight" :disabled="flag =='look'"></uni-easyinput>
						</uni-forms-item>
						<uni-forms-item label="年龄（岁）" required>
							<uni-easyinput v-model="OSTAForm.age" :inputBorder="false" placeholder="请输入年龄"
								@iconClick="clickAge" :disabled="flag =='look'"></uni-easyinput>
						</uni-forms-item>
					...
					<uni-forms label-position="top" :modelValue="riskProfileForm" ref="form4" labelWidth="auto">
						<uni-forms-item label="1、年龄或出生日期（40-90年之间）" required>
							<view class="riskProfilAge">
								<uni-easyinput v-model="riskProfileForm.age" placeholder="请输入年龄" :disabled="flag =='look'"></uni-easyinput>
							</view>
							<uni-datetime-picker type="date" v-model="riskProfileForm.birthday" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="2、性别" required>
							<uni-data-checkbox v-model="riskProfileForm.sex" :localdata="sexs" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="3、体重 (kg)" required>
							<uni-easyinput v-model="riskProfileForm.age" placeholder="请输入体重" :disabled="flag =='look'"></uni-easyinput>
						</uni-forms-item>
						<uni-forms-item label="4、身高 (cm)" required>
							<uni-easyinput v-model="riskProfileForm.height" placeholder="请输入身高" :disabled="flag =='look'">
							</uni-easyinput>
						</uni-forms-item>
						<uni-forms-item label="5、既往骨折史" required>
							<uni-data-checkbox v-model="riskProfileForm.form5" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="6、父母髋骨骨折" required>
							<uni-data-checkbox v-model="riskProfileForm.form6" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="7、目前抽烟行为" required>
							<uni-data-checkbox v-model="riskProfileForm.form7" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="8、肾上腺皮质激素服用" required>
							<uni-data-checkbox v-model="riskProfileForm.form8" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="9、风湿性关节炎" required>
							<uni-data-checkbox v-model="riskProfileForm.form9" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="10、继发性骨质疏松症" required>
							<uni-data-checkbox v-model="riskProfileForm.form10" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="11、每日酒精摄取量达3个单位或以上" required>
							<uni-data-checkbox v-model="riskProfileForm.form11" :localdata="whetherNotList" :disabled="flag =='look'"/>
						</uni-forms-item>
						<uni-forms-item label="12、骨密度 (BMD)" required>
							<uni-data-select v-model="riskProfileForm.BMDmodels" placeholder="选择BMD机型"
								:localdata="BMDmodelsList" @change="change" :disabled="flag =='look'"></uni-data-select>
							<uni-easyinput v-model="riskProfileForm.form12" placeholder="请输入身高" :disabled="flag =='look'">
							</uni-easyinput>
						</uni-forms-item>
					</uni-forms>
				</uni-section>
